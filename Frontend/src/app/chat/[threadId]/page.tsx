      ...
      <div className="relative flex min-h-0 flex-1 flex-col">
        ...
        >
          <div className={`${CHAT_COLUMN} pb-6 pt-2`}>
            {(() => {
              const firstDevTutorIndex = messages.findIndex(msg => msg.agent === "DevelopmentTutor");
              
              let devStartIndex = firstDevTutorIndex;
              if (devStartIndex > 0 && messages[devStartIndex - 1].role === "user") {
                devStartIndex--;
              }

              return messages.map((m, i) => {
                const showPhaseDivider = firstDevTutorIndex !== -1 && i === devStartIndex;

                return (
                  <div key={m.id ? m.id : `msg-${i}-${m.created_at || ""}`}>
                    
                    {showPhaseDivider && (
                      <div className="my-10 flex flex-col items-center justify-center gap-8">
                        
                        {/* Planning Complete Card */}
                        <div className="w-full max-w-lg rounded-3xl border border-emerald-500/25 bg-gradient-to-b from-emerald-500/[0.05] to-transparent p-6 text-center">
                          <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-emerald-500/20 text-emerald-300">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                              <path d="M20 6L9 17l-5-5"/>
                            </svg>
                          </div>
                          <h3 className="text-[16px] font-semibold text-emerald-100">
                            Architecture Plan Validated
                          </h3>
                          <p className="mt-2 text-[13px] leading-relaxed text-[#a0a0a0]">
                            The Auditor agent has approved the final architectural plan. You can review the complete specification before starting implementation.
                          </p>
                          <div className="mt-5 flex justify-center">
                            <button
                              ...
                              className="rounded-full border border-emerald-500/40 bg-emerald-500/10 px-5 py-2.5 text-[13px] font-semibold text-emerald-200 transition-all hover:bg-emerald-500/20 hover:text-emerald-100"
                            >
                        ...
                        {/* The Original Phase Divider */}
                        <div className="flex w-full items-center justify-center gap-4">
                          <div className="h-px flex-1 bg-white/[0.08]" />
                          <span className="text-[10px] font-semibold uppercase tracking-widest text-[#808080]">
                    ...
                    <article className="mb-8 md:mb-8">
                      {m.role === "user" ? (
                                  ...
                                  fv.active_in_family !== false && (
                                    <div className="mt-2 flex max-w-[80%] items-center justify-end gap-3 text-[13px] text-[#a0a0a0]">
                                      ...
                                      <button
                                        type="button"
                                        disabled={
                                          loading || fv.current_index >= fv.total - 1
                                        }
                                        onClick={() => switchForkVersion(forkKey, 1)}
                                        className="rounded-lg p-1 hover:bg-white/[0.06] hover:text-white disabled:opacity-25"
